...
type
  TSynCustomAsmHighlighter = class(TSynCustomHighlighter)
  private
...
procedure TSynCustomAsmHighlighter.DoAddKeywordOper(AKeyword: UnicodeString;
  AKind: integer);
var
  HashValue: Cardinal;
begin
...
function TSynCustomAsmHighlighter.HashKey(Str: PWideChar): Cardinal;
begin
  Result := 0;
...
function TSynCustomAsmHighlighter.IdentKind(MayBe: PWideChar): TtkTokenKind;
var
  Entry: TSynHashEntry;
begin
  fToIdent := MayBe;
...
constructor TSynCustomAsmHighlighter.Create(const Comand, ComandFPU, Dir, Jump, Regist,
...
destructor TSynCustomAsmHighlighter.Destroy;
begin
  fKeywordsOper.Free;
  inherited Destroy;
end;

procedure TSynCustomAsmHighlighter.CommentProc;
begin
  S := '';
  fTokenId := tkComment;
  repeat
...
procedure TSynCustomAsmHighlighter.CRProc;
begin
  fTokenId := tkSpace;
  inc(Run);
...
procedure TSynCustomAsmHighlighter.GreaterProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
  if fLine[Run] = '=' then
...
procedure TSynCustomAsmHighlighter.IdentProc;
begin
  S := '';
...
procedure TSynCustomAsmHighlighter.LFProc;
begin
  fTokenId := tkSpace;
  inc(Run);
end;

procedure TSynCustomAsmHighlighter.LowerProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
...
procedure TSynCustomAsmHighlighter.NullProc;
begin
  fTokenId := tkNull;
  inc(Run);
end;

procedure TSynCustomAsmHighlighter.NumberProc;
...
procedure TSynCustomAsmHighlighter.SlashProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
end;

procedure TSynCustomAsmHighlighter.SpaceProc;
begin
  fTokenId := tkSpace;
  repeat
    inc(Run);
...
procedure TSynCustomAsmHighlighter.StringProc;
begin
  fTokenId := tkString;
...
procedure TSynCustomAsmHighlighter.SetSelectWord(const Value: string);
begin
  fSelectWord := LowerCase(Value);
end;

procedure TSynCustomAsmHighlighter.SetUnderscoreWord(const Value: string);
begin
  fUnderscoreWord := LowerCase(Value);
end;

function TSynCustomAsmHighlighter.equalsUnderscoreWord(const Value: string): Boolean;
begin
  Result := (fUnderscoreWord = LowerCase(Value))
end;

procedure TSynCustomAsmHighlighter.SingleQuoteStringProc;
begin
  fTokenId := tkString;
...
procedure TSynCustomAsmHighlighter.SymbolProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
end;

procedure TSynCustomAsmHighlighter.UnknownProc;
begin
  inc(Run);
  fTokenId := tkIdentifier;
end;

procedure TSynCustomAsmHighlighter.Next;
var
  S: string;
begin
  fTokenPos := Run;
...
function TSynCustomAsmHighlighter.GetDefaultAttribute(Index: integer)
...
function TSynCustomAsmHighlighter.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TSynCustomAsmHighlighter.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenId of
    tkComment:
      Result := fCommentAttri;
...
function TSynCustomAsmHighlighter.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynCustomAsmHighlighter.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

class function TSynCustomAsmHighlighter.GetLanguageName: string;
begin
  Result := SYNS_LangX86Asm;
end;

function TSynCustomAsmHighlighter.IsFilterStored: Boolean;
begin
  Result := fDefaultFilter <> SYNS_FilterX86Assembly;
end;

function TSynCustomAsmHighlighter.GetSampleSource: UnicodeString;
begin
...
class function TSynCustomAsmHighlighter.GetFriendlyLanguageName: UnicodeString;
begin
